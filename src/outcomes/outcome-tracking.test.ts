...
describe("outcome-tracking", () => {
  ...
  describe("getPostsDueForOutcome", () => {
    it("returns due posts with asset and coingecko_id", async () => {
      const pool = {
  ...
  describe("runOutcomeTrackingForWindow", () => {
    ...
    beforeEach(() => {
      pool = {
        query: vi.fn()
          .mockResolvedValueOnce({ rows: [{ post_id: 1, asset_id: 1, price_t0: "100", coingecko_id: "bitcoin" }] })
          .mockResolvedValue({ rowCount: 1 }),
      } as unknown as Pool;
      vi.mocked(getCurrentPriceUsd).mockReset().mockResolvedValue(105);
    });

    it("fetches price, computes pct_delta, inserts outcome", async () => {
      const { processed, errors } = await runOutcomeTrackingForWindow(pool, "1h");
      expect(processed).toBe(1);
      expect(errors).toBe(0);
      expect(getCurrentPriceUsd).toHaveBeenCalledWith("bitcoin");
      expect(pool.query).toHaveBeenCalledWith(
      ...
      pool = {
        query: vi.fn()
          .mockResolvedValueOnce({
            rows: [{ post_id: 2, asset_id: 2, price_t0: "1", coingecko_id: null }],
          })
          .mockResolvedValue({ rowCount: 1 }),
      } as unknown as Pool;
    ...
    it("counts errors when getCurrentPriceUsd returns null", async () => {
      vi.mocked(getCurrentPriceUsd).mockResolvedValue(null);
      const { processed, errors } = await runOutcomeTrackingForWindow(pool, "1h");
      expect(processed).toBe(0);
      expect(errors).toBe(1);
    });
  ...
  describe("runOutcomeTracking", () => {
    it("runs for all three windows", async () => {
      ...
      const intervals = vi.mocked(pool.query).mock.calls
        ...
        .map((c) => c[1][0]);
      expect(intervals).toEqual(["1 hour", "4 hours", "24 hours"]);
    });
  });
});
